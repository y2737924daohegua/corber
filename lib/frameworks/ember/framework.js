...
const ValidatePluginTask       = require('../../tasks/validate/plugin');
const ValidatePlatformTask     = require('../../tasks/validate/platform');
const ValidateAllowNavigation  = require('../../tasks/validate/allow-navigation');
...
module.exports = Framework.extend({
  ...
  validateServe(project, options) {
    ...
    let validatePlugin = new ValidatePluginTask({
      project: project,
      platform: options.platform,
      pluginName: 'cordova-plugin-whitelist'
    });
    validations.push(validatePlugin.run());
