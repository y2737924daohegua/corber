...
//TODO - validate whitelist plugin require dfor android
//TODO - should not need to pass project to targets
//TODO - android install fails if app already exists (need to uninstall)
//TODO - duplicated init structure in ios/android targets
//TODO - android target apkPath needs to consider all options
//TODO - android launch app hangs spawn - server does not start
  ...
  run(opts) {
    ...
    return this.selectEmulator(opts).then((emulator) => {
      ...
      if (emulator.platform === 'ios') {
        buildTarget = new IOSTarget({
          emulator: emulator,
          project: this.project
        });
      } else if (emulator.platform === 'android') {
        buildTarget = new AndroidTarget({
          emulator: emulator,
          project: this.project
        });
      }
      ...
      return framework.validateServe(opts)
        ...
        .then(() => buildTarget.build())
        .then(() => hook.run('afterBuild'))
        .then(() => buildTarget.run())
